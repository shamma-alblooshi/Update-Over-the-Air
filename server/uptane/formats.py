...
#     vin = VIN_SCHEMA)
DUID_SCHEMA = SCHEMA.AnyString() # Instead, for now, we'll go with an ecu serial number.
...
DRONE_VERSION_MANIFEST_SCHEMA = SCHEMA.Object(
    duid = DUID_SCHEMA,
...
SIGNABLE_DRONE_VERSION_MANIFEST_SCHEMA = SCHEMA.Object(
    object_name = 'SIGNABLE_DRONE_VERSION_MANIFEST_SCHEMA',
    signed = DRONE_VERSION_MANIFEST_SCHEMA,
...
# Implementation Specification, but the signed contents of that object.
SWARM_VERSION_MANIFEST_SCHEMA = SCHEMA.Object(
    suid = SUID_SCHEMA, # Spec: vehicleIdentifier
    primary_fogdrone = DUID_SCHEMA, # Spec: primaryIdentifier
    drone_version_manifests = SCHEMA.DictOf(
        key_schema = DUID_SCHEMA,
        value_schema = SCHEMA.ListOf(SIGNABLE_DRONE_VERSION_MANIFEST_SCHEMA)))

# This object corresponds to "VehicleVersionManifest" in ASN.1 in the Uptane
# Implementation Specification.
SIGNABLE_SWARM_VERSION_MANIFEST_SCHEMA = SCHEMA.Object(
    object_name = 'SIGNABLE_SWARM_VERSION_MANIFEST_SCHEMA',
    signed = SWARM_VERSION_MANIFEST_SCHEMA,
    signatures = SCHEMA.ListOf(SIGNATURE_SCHEMA))
...
VEHICLE_REPORT_TO_DIRECTOR_SCHEMA = SCHEMA.Object(
    suid = SUID_SCHEMA,
    software_manifest = SWARM_VERSION_MANIFEST_SCHEMA)
...
ANY_UPTANE_METADATA_SCHEMA = SCHEMA.OneOf([
    ...
    SWARM_VERSION_MANIFEST_SCHEMA,
    DRONE_VERSION_MANIFEST_SCHEMA])

ANY_SIGNABLE_UPTANE_METADATA_SCHEMA = SCHEMA.OneOf([
    SIGNABLE_TIMESERVER_ATTESTATION_SCHEMA,
    SIGNABLE_SWARM_VERSION_MANIFEST_SCHEMA,
    SIGNABLE_DRONE_VERSION_MANIFEST_SCHEMA])
