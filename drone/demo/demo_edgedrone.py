...
ds.generate_signed_drone_manifest()   # saved as ds.most_recent_signed_manifest
ds.submit_drone_manifest_to_primary() # optionally takes different signed manifest
...
most_recent_signed_drone_manifest = None
...
def clean_slate(
  ...
  # Set starting firmware fileinfo (that this drone had come from the factory)
  ...
  # Prepare this Drone's key.
  load_or_generate_key(use_new_keys)

  # Generate a trusted initial time for the Edge drone.
  ...
  uptane.common.create_directory_structure_for_client(
      ...
      demo.DIRECTOR_REPO_NAME: os.path.join(demo.DIRECTOR_REPO_DIR, suid,
  ...
  try:
    ...
    print('Registration with Director failed. Now assuming this Edge Drone is '
        'already registered.')

  try:
    register_self_with_fogdrone()
  except xmlrpc_client.Fault:
    print('Registration with Fog Drone failed. Now assuming this Edge Drone is '
        'already registered.')


  print('\n' + GREEN + ' Now simulating a Edge drone that rolled off the '
      'assembly line\n and has never seen an update.' + ENDCOLORS)
  print("Generating this Edge Drones's first Drone Version Manifest and sending "
      "it to the Fog Drone.")

  generate_signed_drone_manifest()
  submit_drone_manifest_to_primary()
...
def create_secondary_pinning_file():
  ...
  to the client's own directory. (The TUF repository that a Edgedrone points
  to is local, retrieved from the Fog drone and placed in the Edge drone itself
...
def submit_drone_manifest_to_primary(signed_drone_manifest=None):

  global most_recent_signed_drone_manifest
  if signed_drone_manifest is None:
    signed_drone_manifest = most_recent_signed_drone_manifest
  ...
  if tuf.conf.METADATA_FORMAT == 'der':
    ...
    signed_drone_manifest = xmlrpc_client.Binary(signed_drone_manifest)
  ...
  server.submit_drone_manifest(
      edgedrone.suid,
      edgedrone.duid,
      edgedrone.nonce_next,
      signed_drone_manifest)
...
def load_or_generate_key(use_new_keys=False):
  """Load or generate a drone's private key."""
  ...
  drone_key = uptane.common.canonical_key_from_pub_and_pri(key_pub, key_pri)
...
def update_cycle():
  """
  Updates our metadata and images from the Fog drone. Raises the appropriate
  ...
  # Download the time attestation from the Fogdrone.
  time_attestation = pserver.get_time_attestation_for_drone(_duid)
  if tuf.conf.METADATA_FORMAT == 'der':
  ...
  # Download the metadata from the Fogdrone in the form of an archive. This
  # returns the binary data that we need to write to file.
  metadata_archive = pserver.get_metadata(edgedrone.duid)
  ...
  try:
    edgedrone.update_time(time_attestation)
  except uptane.BadTimeAttestation as e:
    print("Timeserver attestation from Fogdrone does not check out: "
        "This Edgedrone's nonce was not found. Not updating this Edgedrone's "
        "time this cycle.")
  except tuf.BadSignatureError as e:
    print(RED + "Timeserver attestation from Fog drone did not check out. Bad "
        "signature. Not updating this Edge Drone's time." + ENDCOLORS)
  ...
  if len(edgedrone.validated_targets_for_this_drone) == 0:
    print_banner(BANNER_NO_UPDATE, color=WHITE+BLACK_BG,
        text='No validated targets were found. Either the Director '
        'did not instruct this drone to install anything, or the target info '
  ...
  #elif len(secondary_ecu.validated_targets_for_this_drone) > 1:
  #  assert False, 'Multiple targets for an ECU not supported in this demo.'


  expected_target_info = edgedrone.validated_targets_for_this_drone[-1]
  ...
  if not pserver.update_exists_for_drone(edgedrone.duid):

    print_banner(BANNER_NO_UPDATE, color=WHITE+BLACK_BG,
        text='Fog drone reports that there is no update for this drone.')
    # print(YELLOW + 'Primary reports that there is no update for this ECU.')
    (image_fname, image) = pserver.get_image(edgedrone.duid)
    generate_signed_drone_manifest()
    submit_drone_manifest_to_primary()
    return
  ...
  if image is None:
    print(YELLOW + 'Requested image from Fogdrone but received none. Update '
        'terminated.' + ENDCOLORS)
    attacks_detected += 'Requested image from Fogdrone but received none.\n'
    generate_signed_drone_manifest()
    submit_drone_manifest_to_primary()
    return

  elif not edgedrone.validated_targets_for_this_drone:
    print(RED + 'Requested and received image from Fogdrone, but metadata '
        'indicates no valid targets from the Director intended for this drone. '
    ...
    print(RED + 'Requested and received image from Fogdrone, but this '
        'Edgedrone has not validated any target info that matches the given ' +
    ...
    attacks_detected += 'Received unexpected image from Fogdrone with ' + \
        'unexpected filename.\n'
    generate_signed_drone_manifest()
    submit_drone_manifest_to_primary()
    return
  ...
  try:
    edgedrone.validate_image(image_fname)
  except tuf.DownloadLengthMismatchError:
    print_banner(
        BANNER_DEFENDED, color=WHITE+DARK_BLUE_BG,
        text='Image from Fogdrone failed to validate: length mismatch. Image: ' +
        repr(image_fname), sound=TADA)
    # TODO: Add length comparison instead, from error.
    attacks_detected += 'Image from Fogdrone failed to validate: length ' + \
        'mismatch.\n'
    generate_signed_drone_manifest()
    submit_drone_manifest_to_primary()
    return
    ...
    print_banner(
        BANNER_DEFENDED, color=WHITE+DARK_BLUE_BG,
        text='Image from Fogdrone failed to validate: hash mismatch. Image: ' +
        repr(image_fname), sound=TADA)
    # TODO: Add hash comparison instead, from error.
    attacks_detected += 'Image from Fogdrone failed to validate: hash ' + \
        'mismatch.\n'
    generate_signed_drone_manifest()
    submit_drone_manifest_to_primary()
    return
  ...
  if edgedrone.firmware_fileinfo == expected_target_info:
    print_banner(
  ...
  #    new firmware.
  edgedrone.firmware_fileinfo = expected_target_info
...
def generate_signed_drone_manifest():
  ...
  most_recent_signed_drone_manifest = edgedrone.generate_signed_drone_manifest(
      attacks_detected)
...
def ATTACK_send_corrupt_manifest_to_primary():
  ...
  import copy
  corrupt_signed_manifest = copy.copy(most_recent_signed_drone_manifest)
  ...
  try:
    submit_drone_manifest_to_primary(corrupt_signed_manifest)
  except xmlrpc_client.Fault:
...
def register_self_with_director():
  """
  Send the Director a message to register our duid and Public Key.
  ...
  print('Registering Edge Drone Duid and Key with Director.')
  server.register_duid(
      edgedrone.duid,
      uptane.common.public_key_from_canonical(edgedrone.drone_key), _suid,
      False)
  print(GREEN + 'Edge drone has been registered with the Director.' + ENDCOLORS)





def register_self_with_fogdrone():
  """
  ...
  print('Registering Edge drone and Key with Fog drone.')
  server.register_new_edgedrone(edgedrone.duid)
  print(GREEN + 'Edge drone has been registered with the Fog drone.' + ENDCOLORS)
