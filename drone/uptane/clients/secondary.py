...
class Edgedrone(object):
  ...
  <Fields>
    ...
    self.suid
      A unique identifier for the swarm that contains this Edgedrone.
      In this reference implementation, this conforms to
      uptane.formats.SUID_SCHEMA. 

    self.duid
      A unique identifier for this edge drone. In this reference
    ...
    self.drone_key:
      The signing key for this edge drone. This key will be used to sign
      Drone Manifests that will then be sent along to the fog drone (and
      subsequently to the Director). The Director should be aware of the
      corresponding public key, so that it can validate these drone Manifests.
    ...
    self.full_client_dir:
      ...
      edgedrone. This includes verified and unverified metadata and images and
    ...
    self.director_public_key
      If this is a partial verification edgdrone, we store the key that we
    ...
    self.firmware_fileinfo:
      The target file info for the image this Edgedrone is currently using
      (has currently "installed"). This is generally filename, hash, and
      length. See tuf.formats.TARGETFILE_SCHEMA, which contains
      tuf.formats.FILEINFO_SCHEMA. This info is provided in Drone Manifests
      generated for the Director's consumption.

    self.nonce_next
      Next nonce the drone will send to the Timeserver (via the fogdrone).

    self.last_nonce_sent
      The latest nonce this drone sent to the Timeserver (via the fogdrone).
    ...
    self.validated_targets_for_this_drone:
      A list of the targets validated for this drone, populated in method
  ...
  def __init__(
    ...
    <Arguments>
      ...
      suid                   See class docstring above.

      duid            See class docstring above.

      drone_key               See class docstring above.
    ...
    if not self.partial_verifying and self.director_public_key is not None:
      raise uptane.Error('Edge drone not set as partial verifying, but a director ' # TODO: Choose error class.
          'key was still provided. Full verification edgedrones employ the '
          'normal TUF verifications rooted at root metadata files.')

    elif self.partial_verifying and self.director_public_key is None:
      raise uptane.Error('Edge Drone set as partial verifying, but a director '
          'key was not provided. Partial verification edgedrone validate '
          'only the ')
  ...
  def generate_signed_drone_manifest(self, description_of_attacks_observed=''):
    """
    Returns a signed drone manifest indicating self.firmware_fileinfo.
    ...
    # We'll construct a signed signable_drone_manifest_SCHEMA from the
    # targetinfo.
    # First, construct and check an DRONE_VERSION_MANIFEST_SCHEMA.
    drone_manifest = {
        'duid': self.duid,
    ...
    uptane.formats.DRONE_VERSION_MANIFEST_SCHEMA.check_match(drone_manifest)

    # Now we'll convert it into a signable object and sign it with a key we
    # generate.

    # Wrap the ECU version manifest object into an
    # uptane.formats.SIGNABLE_DRONE_VERSION_MANIFEST_SCHEMA and check the format.
    # {
    #     'signed': ecu_version_manifest,
    #     'signatures': []
    # }
    signable_drone_manifest = tuf.formats.make_signable(drone_manifest)
    uptane.formats.SIGNABLE_DRONE_VERSION_MANIFEST_SCHEMA.check_match(
        signable_drone_manifest)

    if tuf.conf.METADATA_FORMAT == 'der':
      der_signed_drone_manifest = asn1_codec.convert_signed_metadata_to_der(
          signable_drone_manifest, DATATYPE_DRONE_MANIFEST, resign=True,
          private_key=self.drone_key)
      # TODO: Consider verification of output here.
      return der_signed_drone_manifest

    # Else use standard Python dictionary format specified in uptane.formats.

    # Now sign with that key.
    uptane.common.sign_signable(
        signable_drone_manifest, [self.drone_key], DATATYPE_DRONE_MANIFEST)
    uptane.formats.SIGNABLE_DRONE_VERSION_MANIFEST_SCHEMA.check_match(
        signable_drone_manifest)

    return signable_drone_manifest
  ...
  def update_time(self, timeserver_attestation):
    ...
    distributed to us by the Fogdrone.
    ...
    in a request for the time), then this Edgedrones's time is updated.
    The new time will be used by this client (via TUF) in in place of system
    time when checking metadata for expiration.

    If the Edgedrone is using ASN.1/DER metadata, then timeserver_attestation
    ...
    if self.last_nonce_sent is None:
      ...
      log.warning(YELLOW + 'Cannot verify a timeserver attestation yet: '
          'this fresh Edge drone has never communicated a nonce and Drone '
          'Version Manifest to the Fogdrone.' + ENDCOLORS)
      return
      ...
      raise uptane.BadTimeAttestation('Fog drone provided a time attestation '
          'that did not include any of the nonces this Edge Drone has sent '
          'recently. This Fogdrone cannot trust the time provided and will '
          'not register it. Because of the asynchrony in the '
          ...
          'Fog drone is compromised or that there is a Man in the Middle attack '
          'underway between the swarm and the servers, or within the '
          'swarm.')
  ...
  def fully_validate_metadata(self):
    ...
    validating it against the older metadata this Edgedrone already has and
    already validated.
    ...
    self.validated_targets_for_this_drone if Director and Image repositories
    ...
    validated_targets_for_this_drone = []
    ...
    for target in self.updater.targets_of_role(
      ...
      if 'custom' not in target['fileinfo'] or \
          'duid' not in target['fileinfo']['custom'] or \
          self.duid != target['fileinfo']['custom']['duid']:
        continue

      # Fully validate the target info for our target(s).
      try:
        validated_targets_for_this_drone.append(
        ...
        log.error(RED + 'Unable to validate target ' +
            ...
            'Edgedrone , using the validation rules in pinned.json' +
            ENDCOLORS)
        continue


    self.validated_targets_for_this_drone = validated_targets_for_this_drone
  ...
  def process_metadata(self, metadata_archive_fname):
    ...
    Pick out the target file(s) with our DUID listed
  ...
  def _expand_metadata_archive(self, metadata_archive_fname):
    ...
    distribution of these metadata files from Fogdrone to Edgedrone will vary
  ...
  def validate_image(self, image_fname):
    ...
    fully_validate_metadata, stored in self.validated_targets_for_this_drone). If
    ...
    for targetinfo in self.validated_targets_for_this_drone:
