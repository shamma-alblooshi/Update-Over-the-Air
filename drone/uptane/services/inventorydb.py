...
def save_vehicle_manifest(suid, signed_vehicle_manifest):
  """
  ...
  """
  check_vin_registered(suid) # check arg format and registration

  uptane.formats.SIGNABLE_VEHICLE_VERSION_MANIFEST_SCHEMA.check_match(
       signed_vehicle_manifest)

  vehicle_manifests[suid].append(signed_vehicle_manifest)
...
def register_ecu(is_primary, suid, ecu_serial, public_key, overwrite=True):
  """
  ...
  if not overwrite:
    ...
    if is_primary and suid in primary_ecus_by_vin and \
        primary_ecus_by_vin[suid] is not None:
  ...
  # registered.
  check_vin_registered(suid)


  # Associate the ECU with the vehicle.
  if ecu_serial not in ecus_by_vin[suid]:
    ecus_by_vin[suid].append(ecu_serial)

  if is_primary:
    # Set the ECU as the vehicle's Primary ECU.
    primary_ecus_by_vin[suid] = ecu_serial
...
def register_vehicle(suid, primary_ecu_serial=None, overwrite=True):
  ...
  if not overwrite and suid in ecus_by_vin:
    raise uptane.Spoofing('The given VIN, ' + repr(suid) + ', is already '
        'registered.')

  ecus_by_vin[suid] = []
  vehicle_manifests[suid] = []
  primary_ecus_by_vin[suid] = primary_ecu_serial
...
def _check_registration_is_sane(suid):
  ...
  assert (suid in vehicle_manifests) == (suid in ecus_by_vin) == (
      suid in primary_ecus_by_vin), 'Programming error.'
