...
class Fogdrone(object): # Consider inheriting from Secondary and refactoring.
  """
  <Purpose>
    ...
    metadata and images to Edge drones, combining DRONE Manifests into a
    SWARM Manifest and signing it, combining tokens for a Timeserver request,
    validating the response, etc.

  <Fields>

    self.suid
      A unique identifier for the swarm that contains this edge drones.
      In this reference implementation, this conforms to
      uptane.formats.SUID_SCHEMA. There is no need to use the swarms's SUID in
      particular; we simply need a unique identifier for the swarm, known
      to the Director.

    self.duid
      A unique identifier for this fog drone. In this reference
      implementation, this conforms to uptane.formats.DUID_SCHEMA.
    ...
    self.fogdrone_key
      The signing key for this Fog Drone. This key will be used to sign
      SWARM Manifests that will then be sent to the Director). The Director
      should be aware of the corresponding public key, so that it can validate
      these Swarm Manifests. Conforms to tuf.formats.ANYKEY_SCHEMA.
    ...
    self.drone_manifests
      A dictionary containing the manifests provided by all drones. Will include
      all manifests sent by all drones. The Primary does not verify signatures on
    ...
    self.my_edgedrones:
      This is a list of all duids belonging to the EdgeDrone's of this
      Fog drone.
  ...
  def __init__(
    ...
    tuf.formats.ANYKEY_SCHEMA.check_match(fogdrone_key)
    ...
    self.fogdrone_key = fogdrone_key
    self.my_edgedrones= my_edgedrones
    if self.my_edgedrones is None:
      self.my_edgedrones = [] # (because must not use mutable as default value)
  ...
  def get_target_list_from_director(self):
    ...
    """
    # This will have to be changed (along with the functions that depend
  ...
  def fogdrone_update_cycle(self):
    """
    ...
    if not directed_targets:
      log.info('A correctly signed statement from the Director indicates that '
          'this swarm has NO updates to install.')
    else:
      log.info('A correctly signed statement from the Director indicates that '
          'this swarm has updates to install:' +
    ...
    for target in verified_targets:
      ...
      if 'custom' not in target['fileinfo'] or \
          'duid' not in target['fileinfo']['custom']:
        raise uptane.Error('Director repo failed to include a DUID for '
            'a target. Target metadata was: ' + repr(target))

      # Get the ECU Serial listed in the custom file data.
      assigned_duid = target['fileinfo']['custom']['duid']

      # Make sure it's actually an ECU we know about.
      if assigned_duid not in self.my_edgedrones:
      ...
      self.assigned_targets[assigned_duid] = target
      ...
      try:
        ...
        log.info(YELLOW + 'The Director and Image Repository provided '
            ...
            'rejected' + ENDCOLORS + ' Swarm not updated.')
  ...
  def get_image_fname_for_drone(self, duid):
    """
  ...
  def update_exists_for_drone(self, duid):
    """
    Returns True if the Director has sent us instructions for the edgedrone
    ...
    if duid not in self.my_edgedrones:
      raise uptane.UnknownECU(
          'Received a request for an update for a Fogdrone (' +
          repr(duid) + ') of which this Edgedrone is not aware.')

    elif duid not in self.assigned_targets:
      log.info(
          'Received request for an update for a Fogdrone(' +
          repr(duid) + ') for which this Edgedrone has no update '
  ...
  def generate_signed_swarm_manifest(self):
    """
    ...
    swarm_manifest = {
        'suid': self.suid,
        'fogdrone_duid': self.duid,
        'drone_version_manifests': self.drone_manifests
    }

    uptane.formats.SWARM_VERSION_MANIFEST_SCHEMA.check_match(swarm_manifest)
    ...
    #     'signatures': []
    # }
    signable_swarm_manifest = tuf.formats.make_signable(swarm_manifest)
    uptane.formats.SIGNABLE_SWARM_VERSION_MANIFEST_SCHEMA.check_match(
        signable_swarm_manifest)

    if tuf.conf.METADATA_FORMAT == 'der':
      # Convert to DER and sign, replacing the Python dictionary.
      signable_swarm_manifest = asn1_codec.convert_signed_metadata_to_der(
          signable_swarm_manifest, DATATYPE_SWARM_MANIFEST,
          private_key=self.fogdrone_key, resign=True)
    ...
    self.drone_manifests = dict()

    return signable_swarm_manifest





  def register_new_edgedrone(self, duid):
    """
    ...
    if duid in self.my_edgedrones:
      log.info('DUID' + repr(duid) + ' already registered with '
          'this Fogdrone.')
      return

    self.my_edgedrones.append(duid)
    log.debug('DUID ' + repr(duid) + ' has been registered as '
        'an Edgedrone with this Fogdrone.')





  def _check_duid(self, duid):
    """
    ...
    if duid not in self.my_edgedrones:
      raise uptane.UnknownECU("The given drone is not in this Fog Drone's list of "
          "known Edge Drones. Register the drone with this Fog drone first.")




  def register_drone_manifest(
      self, suid, duid, nonce, signed_drone_manifest, force_pydict=False):
    """
    <Purpose>
    ...
    <Arguments>
      suid
          See class docstring above. The SUID of a Secondary in this vehicle
          submitting an ECU Manifest is expected to be the same as the SUID for
      ...
      signed_drone_manifest
          ...
          The expected format that signed_drone_manifest should conform to is
      ...
      force_pydict (optional, default False)
          When True, the function treats signed_drone_manifest as if the value of
          tuf.conf.METADATA_FORMAT is set to 'json'. See signed_drone_manifest.

    <Exceptions>

      uptane.Spoofing
          if duid is not the same as the ECU Serial listed in the
          provided ECU Manifest itself.

      uptane.UnknownECU
          if duid is not one of this Primary's Secondaries

      uptane.UnknownVehicle
          if the SUID argument is not the same as this primary's SUID
    ...
    if suid != self.suid:
      raise uptane.UnknownVehicle('Received a Drone Manifest supposedly hailing '
          'from a different swarm....')

    if tuf.conf.METADATA_FORMAT == 'der' and not force_pydict:
      uptane.formats.DER_DATA_SCHEMA.check_match(signed_drone_manifest)
      ...
      signed_drone_manifest = asn1_codec.convert_signed_der_to_dersigned_json(
          signed_drone_manifest, DATATYPE_DRONE_MANIFEST)

    # Else, we're working with standard Python dictionaries and no conversion
    # is necessary, but we'll still validate the signed_drone_manifest argument.
    else:
      uptane.formats.SIGNABLE_SWARM_VERSION_MANIFEST_SCHEMA.check_match(
          signed_drone_manifest)
    ...
    # the manifest to the Fogdrone's dictionary of manifests.
    if duid in self.drone_manifests:
      self.drone_manifests[duid].append(signed_drone_manifest)
    else:
      self.drone_manifests[duid] = [signed_drone_manifest]

    # And add the nonce the Edgedrone provided to the list of nonces to send
    # in the next Timeserver request.
    if nonce not in self.nonces_to_send:
      self.nonces_to_send.append(nonce)


    log.debug(GREEN + ' Fogdrone received a drone manifest from edgedrone ' +
        repr(duid) + ', along with nonce ' + repr(nonce) + ENDCOLORS)

    # Alert if there's been a detected attack.
    if signed_drone_manifest['signed']['attacks_detected']:
      log.warning(YELLOW + ' Attacks have been reported by the Edgedrone! \n '
          'Attacks listed by drone ' + repr(duid) + ':\n ' +
