...
class Director:
  ...
  def register_ecu_serial(self, ecu_serial, ecu_key, suid, is_primary=False):
    """
    ...
    inventory.check_vin_registered(suid)

    # Register the public key and associate the ECU with the given VIN.
    inventory.register_ecu(
        is_primary, suid, ecu_serial, ecu_key, overwrite=False)

    log.info(
        GREEN + 'Registered a new ECU, ' + repr(ecu_serial) + ' in '
        'vehicle ' + repr(suid) + ' with ECU public key: ' + repr(ecu_key) +
        ENDCOLORS)
  ...
  def register_vehicle_manifest(
      self, suid, primary_ecu_serial, signed_vehicle_manifest):
    """
    ...
    """
    uptane.formats.SUID_SCHEMA.check_match(suid)
    uptane.formats.DUID_SCHEMA.check_match(primary_ecu_serial)
    ...
    if tuf.conf.METADATA_FORMAT == 'der':
      ...
      signed_vehicle_manifest = asn1_codec.convert_signed_der_to_dersigned_json(
          signed_vehicle_manifest, DATATYPE_SWARM_MANIFEST)

    uptane.formats.SIGNABLE_VEHICLE_VERSION_MANIFEST_SCHEMA.check_match(
        signed_vehicle_manifest)

    if suid not in inventory.ecus_by_vin:
    ...
    self.validate_primary_certification_in_vehicle_manifest(
        suid, primary_ecu_serial, signed_vehicle_manifest)

    # If the Primary's signature is valid, save the whole vehicle manifest to
    # the inventorydb.
    inventory.save_vehicle_manifest(suid, signed_vehicle_manifest)
    ...
    for ecu_serial in all_ecu_manifests:
      ...
      for manifest in ecu_manifests:
        try:
          ...
          # caught below.
          self.register_ecu_manifest(suid, ecu_serial, manifest)
  ...
  def validate_primary_certification_in_vehicle_manifest(
      self, suid, primary_ecu_serial, vehicle_manifest):
    """
    ...
    if tuf.conf.METADATA_FORMAT == 'der':
      ...
      data_to_check = asn1_codec.convert_signed_metadata_to_der(
          vehicle_manifest, DATATYPE_SWARM_MANIFEST, only_signed=True)
  ...
  def add_new_vehicle(self, suid, primary_ecu_serial=None):
    """
    ...
    inventory.register_vehicle(suid, primary_ecu_serial=primary_ecu_serial)
  ...
  def create_director_repo_for_vehicle(self, suid):
    ...
    self.vehicle_repositories[suid] = this_repo = rt.create_new_repository(
