...
most_recent_signed_swarm_manifest = None
...
def clean_slate(
  ...
  # Generate a trusted initial time for the Fog drone.
  ...
  # Load the private key for this Fog drone.
  ...
  # Initialize a Fog drone, making a client directory and copying the root
  # file from the repositories.
  fogdrone = primary.Fogdrone(
      full_client_dir=os.path.join(uptane.WORKING_DIR, CLIENT_DIRECTORY),
      director_repo_name=demo.DIRECTOR_REPO_NAME,
      suid=_suid,
      duid=_duid,
      fogdrone_key=drone_key,
      time=clock,
  ...
  print('\n' + GREEN + 'Fog drone is now listening for messages from ' +
      'Edge drones.' + ENDCOLORS)


  try:
    register_self_with_director()
  except xmlrpc_client.Fault:
    print('Registration with Director failed. Now assuming this Fog drone is '
        'already registered.')


  print(GREEN + '\n Now simulating a Fog drone that rolled off the assembly line'
      '\n and has never seen an update.' + ENDCOLORS)

  print("Generating this Fog Drone's first Swarm Version Manifest and sending "
      "it to the Director.")

  generate_signed_swarm_manifest()
  submit_swarm_manifest_to_director()
...
def create_primary_pinning_file():
  ...
  mirror = mirror.replace('<VIN>', _suid)
...
def update_cycle():
  ...
  nonces_to_send = fogdrone.get_nonces_to_send_and_rotate()
  ...
  # at will.
  fogdrone.update_time(time_attestation)
  ...
  try:
    fogdrone.fogdrone_update_cycle()
  ...
  # re-raised.
    ...
    director_file = os.path.join(_suid, 'metadata', 'timestamp' + demo.METADATA_EXTENSION)
...
def generate_signed_swarm_manifest():

  global most_recent_signed_swarm_manifest

  # Generate and sign a manifest indicating that this ECU has a particular
  # version/hash/size of file2.txt as its firmware.
  most_recent_signed_swarm_manifest = \
      fogdrone.generate_signed_swarm_manifest()





def submit_swarm_manifest_to_director(signed_swarm_manifest=None):

  global most_recent_signed_swarm_manifest

  if signed_swarm_manifest is None:
    signed_swarm_manifest = most_recent_signed_swarm_manifest
  ...
  if tuf.conf.METADATA_FORMAT == 'der':
    ...
    signed_swarm_manifest = xmlrpc_client.Binary(signed_swarm_manifest)
  ...
  print("Submitting the Fog Drone's manifest to the Director.")

  server.submit_swarm_manifest(
      fogdrone.suid,
      fogdrone.duid,
      signed_swarm_manifest)


  print(GREEN + 'Submission of Swarm Manifest complete.' + ENDCOLORS)
...
def register_self_with_director():
  ...
  print('Registering Fog drone DUID and Key with Director.')
  server.register_duid(
      fogdrone.duid,
      uptane.common.public_key_from_canonical(fogdrone.fogdrone_key),
      _suid, True)
  print(GREEN + 'Fog drone has been registered with the Director.' + ENDCOLORS)
...
def get_image_for_drone(duid):
  """
  Intended to be called via XMLRPC by the Edgedrone client, either partial or
  full verification.
  ...
  fogdrone._check_duid(duid)

  image_fname = fogdrone.get_image_fname_for_drone(duid)

  if image_fname is None:
    print('DUID ' + repr(duid) + ' requested an image, but this '
        'Fogdrone has no update for that drone.')
    return None, None
  ...
  print('Distributing image to drone ' + repr(duid))
  ...
  relative_fname = os.path.relpath(
      image_fname, os.path.join(fogdrone.full_client_dir, 'targets'))
...
def get_metadata_for_drone(duid, force_partial_verification=False):
  """
  Provides the current metadata an Edgedrone will need to validate updates.
  ...
  if force_partial_verification:
    fname = fogdrone.get_partial_metadata_fname()
    ...
    fname = fogdrone.get_full_metadata_archive_fname()

  if not os.path.exists(fname):
    raise uptane.Error(
        'Fogdrone has no metadata to distribute to Edgedrone "' + duid +
        '". Missing filename: "' + fname + '". Currently operating in ' +
        ('Partial' if force_partial_verification else 'Full') +
        ' Verification Mode')

  print('Distributing metadata file ' + fname + ' to drone ' + repr(duid))
...
def get_time_attestation_for_drone(duid):
  """
  """
  # Ensure serial is correct format & registered
  fogdrone._check_duid(duid)

  attestation = fogdrone.get_last_timeserver_attestation()
...
def register_drone_manifest_wrapper(suid, duid, nonce, signed_drone_manifest):
  """
  This function is a wrapper for primary.Primary::register_drone_manifest().
  ...
  """
  if tuf.conf.METADATA_FORMAT == 'der':
    fogdrone.register_drone_manifest(
        suid, duid, nonce, signed_drone_manifest.data)
  else:
    fogdrone.register_drone_manifest(
        suid, duid, nonce, signed_drone_manifest)
...
def listen():
  ...
  for port in demo.PRIMARY_SERVER_AVAILABLE_PORTS:
    try:
      ...
      print('Failed to bind Fog drone XMLRPC Listener to port ' + repr(port) +
          '. Trying next port.')
      last_error = e
  ...
  server.register_function(
      ...
      # XMLRPC.
      register_drone_manifest_wrapper, 'submit_drone_manifest')
  ...
  server.register_function(
      fogdrone.register_new_edgedrone, 'register_new_edgedrone')

  server.register_function(
      get_time_attestation_for_drone, 'get_time_attestation_for_drone')
  ...
  server.register_function(get_image_for_drone, 'get_image')

  server.register_function(get_metadata_for_drone, 'get_metadata')
  ...
  # issue.
  server.register_function(
      fogdrone.update_exists_for_drone, 'update_exists_for_drone')

  # server.register_function(compromise_primary_and_deliver_arbitrary,
  #     'compromise_primary_and_deliver_arbitrary')


  print('Fog drone will now listen on port ' + str(successful_port))
