...
KNOWN_SUIDS = ['111', '112', '113', 'democar']
...
def clean_slate(use_new_keys=False):
  ...
  for suid in KNOWN_SUIDS:
    director_service_instance.add_new_vehicle(suid)
  ...
  for suid in inventory.ecus_by_vin:
    for ecu in inventory.ecus_by_vin[suid]:
...
def write_to_live(suid_to_update=None):
  ...
  for suid in director_service_instance.vehicle_repositories:
    if suid_to_update is not None and suid != suid_to_update:
      continue
    repo = director_service_instance.vehicle_repositories[suid]
...
def add_target_to_director(target_fname, filepath_in_repo, suid, ecu_serial):
  """
  ...
  if suid not in director_service_instance.vehicle_repositories:
    raise uptane.UnknownVehicle('The VIN provided, ' + repr(suid) + ' is not '
  ...
  director_service_instance.add_target_for_ecu(
      suid, ecu_serial, destination_filepath)
...
def register_vehicle_manifest_wrapper(
  ...
  if tuf.conf.METADATA_FORMAT == 'der':
    director_service_instance.register_vehicle_manifest(
        suid, primary_ecu_serial, signed_vehicle_manifest.data)
  else:
    director_service_instance.register_vehicle_manifest(
        suid, primary_ecu_serial, signed_vehicle_manifest)
