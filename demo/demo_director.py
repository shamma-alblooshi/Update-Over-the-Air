...
KNOWN_SUIDS = ['111', '112', '113', 'democar', 'firstswarm']
...
def clean_slate(use_new_keys=False):
  ...
  print(LOG_PREFIX + 'Initializing swarm repositories')
  ...
  for suid in KNOWN_SUIDS:
    director_service_instance.add_new_swarm(suid)
  ...
  for suid in inventory.drones_by_suid:
    for ecu in inventory.drones_by_suid[suid]:
...
def write_to_live(suid_to_update=None):
  ...
  for suid in director_service_instance.swarm_repositories:
    if suid_to_update is not None and suid != suid_to_update:
      continue
    repo = director_service_instance.swarm_repositories[suid]
...
def backup_repositories(vin=None):
  ...
  """
  if vin is None:
    repos_to_backup = director_service_instance.swarm_repositories.keys()
  else:
    repos_to_backup = [vin]

  for vin in repos_to_backup:
    repo = director_service_instance.swarm_repositories[vin]
    repo_dir = repo._repository_directory
...
def restore_repositories(vin=None):
  ...
  """
  if vin is None:
    repos_to_restore = director_service_instance.swarm_repositories.keys()
  else:
    repos_to_restore = [vin]

  for vin in repos_to_restore:

    repo_dir = director_service_instance.swarm_repositories[
    ...
    director_service_instance.swarm_repositories[vin] = rt.load_repository(
        repo_dir)
    ...
    director_service_instance.swarm_repositories[vin].root.load_signing_key(
...
def revoke_compromised_keys():
  ...
  for vin in director_service_instance.swarm_repositories:
    repository = director_service_instance.swarm_repositories[vin]
...
def sign_with_compromised_keys_attack(vin=None):
  ...
  if vin is None:
    vehicles_to_attack = director_service_instance.swarm_repositories.keys()
  else:
...
def add_target_to_director(target_fname, filepath_in_repo, suid, duid):
  """
  ...
  if suid not in director_service_instance.swarm_repositories:
    raise uptane.UnknownVehicle('The SUID provided, ' + repr(suid) + ' is not '
        'that of a swarm known to this Director.')
  ...
  print(LOG_PREFIX + 'Adding target ' + repr(target_fname) + ' for drone ' +
      repr(suid))

  # This calls the appropriate vehicle repository.
  director_service_instance.add_target_for_drone(
      suid, duid, destination_filepath)
...
def host():
  ...
  if sys.version_info.major < 3: # Python 2 compatibility
    ...
    command = ['python3', '-m', 'http.server', str(demo.DIRECTOR_REPO_PORT),'--bind',str(demo.DIRECTOR_REPO_HOST)]
...
def register_swarm_manifest_wrapper(
    suid, fogdrone_duid, signed_swarm_manifest):
  """
  ...
  if tuf.conf.METADATA_FORMAT == 'der':
    director_service_instance.register_swarm_manifest(
        suid, fogdrone_duid, signed_swarm_manifest.data)     
  else:
    director_service_instance.register_swarm_manifest(
        suid, fogdrone_duid, signed_swarm_manifest)    
...
def listen():
  ...
  server.register_function(
      ...
      register_swarm_manifest_wrapper, # due to XMLRPC.Binary() for DER
      'submit_swarm_manifest')

  server.register_function(
      director_service_instance.register_duid, 'register_duid')


  # Interface available for the demo website frontend.
  server.register_function(
      director_service_instance.add_new_swarm, 'add_new_vehicle')
...
def keyed_arbitrary_package_attack(vin, ecu_serial, target_filepath):
  ...
  target_full_path = os.path.join(
      director_service_instance.swarm_repositories[vin]._repository_directory,
      'targets', target_filepath)
...
def clear_vehicle_targets(vin):
  ...
  director_service_instance.swarm_repositories[vin].targets.clear_targets()
