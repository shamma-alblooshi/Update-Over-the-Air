...
def get_asn_signed(json_signed):
  ...
  signed['vehicleIdentifier'] = json_signed['suid']
  signed['primaryIdentifier'] = json_signed['primary_fogdrone']
  ...
  sorted_ecu_serials = sorted(json_signed['drone_version_manifests'])
  for duid in sorted_ecu_serials:
    for manifest in json_signed['drone_version_manifests'][duid]:
...
def get_json_signed(asn_metadata):
  ...
  json_signed = {
      'suid': str(asn_signed['vehicleIdentifier']),
      'primary_fogdrone': str(asn_signed['primaryIdentifier'])
  }
  ...
  for i in range(numberOfECUVersionManifests):
    ...
    duid = json_manifest_signed['duid']
    ...
    if duid not in json_manifests:
      json_manifests[duid] = []
    json_manifests[duid].append(json_manifest)

  json_signed['drone_version_manifests'] = json_manifests
